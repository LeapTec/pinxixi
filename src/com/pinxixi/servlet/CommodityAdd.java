...
public class CommodityAdd extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(comm.AddCommodity(como)) {
			request.getSession().setAttribute("CID", CID);
			PrintWriter out = response.getWriter();
			out.println("<script type=\"text/javascript\"> alert(\"添加商品成功！\"); window.top.location.href=\"商品管理页\"; </script>");
		}else {
			PrintWriter out = response.getWriter();
			out.println("<script type=\"text/javascript\"> alert(\"添加商品失败！\"); window.top.location.href=\"商品管理页\"; </script>");
		}
	}
