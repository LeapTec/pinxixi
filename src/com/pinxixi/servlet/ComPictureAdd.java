...
/**
 * Servlet implementation class ComPictureAdd
 */
@WebServlet("/ComPictureAdd")
public class ComPictureAdd extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public ComPictureAdd() {
        super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(compd.AddComPicture(comp)) {
			request.getSession().setAttribute("CPID", CPID);
			PrintWriter out = response.getWriter();
			out.println("<script type=\"text/javascript\"> alert(\"添加图片成功！\"); window.top.location.href=\"商品图片页\"; </script>");
		}else {
			PrintWriter out = response.getWriter();
			out.println("<script type=\"text/javascript\"> alert(\"添加图片失败！\"); window.top.location.href=\"商品图片页\"; </script>");
		}
	}
